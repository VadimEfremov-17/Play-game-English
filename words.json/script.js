...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Загружаем стандартные слова, если нет сохраненных
...
function updateUIStats() {
    ...
    document.getElementById('count-beginner').textContent = 
        `${appState.words.beginner ? appState.words.beginner.length : 0} слов`;
    document.getElementById('count-intermediate').textContent = 
        `${appState.words.intermediate ? appState.words.intermediate.length : 0} слов`;
    document.getElementById('count-advanced').textContent = 
        `${appState.words.advanced ? appState.words.advanced.length : 0} слов`;
}
...
function showSection(sectionName) {
    ...
    // Особые действия для некоторых секций
...
function startNewGame() {
    ...
    // Установка количества вопросов из настроек
    ...
    document.getElementById('current-level-badge').textContent = 
        gameState.currentLevel === 'beginner' ? 'Начинающий' : 
        gameState.currentLevel === 'intermediate' ? 'Средний' : 'Продвинутый';
...
function generateQuestions() {
    ...
    if (!words || words.length === 0) {
        showNotification('В этом уровне нет слов! Добавьте слова сначала.', 'error');
        showMainMenu();
        return;
    }
...
function showNextQuestion() {
    ...
    // Показ вопроса в зависимости от режима
...
function showTranslationQuestion(fromLang, toLang) {
    ...
    questionContainer.innerHTML = `
        <div class="question-text">Переведите слово: <strong>${questionText}</strong></div>
    `;
...
function showMultipleChoiceQuestion() {
    ...
    // Показ вопроса
    questionContainer.innerHTML = `
        <div class="question-text">Выберите правильный перевод слова: <strong>${gameState.currentWord.en}</strong></div>
    `;
    ...
    // Создаем массив всех вариантов и перемешиваем
...
function showFillBlankQuestion() {
    ...
    // Показ вопроса
    questionContainer.innerHTML = `
        <div class="question-text">Заполните пропуск в предложении:</div>
        <div class="question-example">"${sentenceWithGap}"</div>
        <div class="hint-text">Перевод пропущенного слова: ${gameState.currentWord.ru}</div>
    `;
...
function showDictationQuestion() {
    ...
    // Показ вопроса
    questionContainer.innerHTML = `
        <div class="question-text">Напишите слово на английском:</div>
    ...
    answerContainer.innerHTML = `
        <input type="text" class="input-answer" id="user-answer" 
               placeholder="Введите слово на английском..." autocomplete="off">
    `;
...
function showMixedQuestion() {
    ...
    // Временная установка типа вопроса и отображение
...
function checkAnswer() {
    ...
    // Получение ответа пользователя в зависимости от режима
    switch (gameState.currentMode) {
        case 'translation-ru-en':
        case 'translation-en-ru':
        case 'fill-blank':
        ...
        case 'mixed':
            // Для смешанного режима проверяем в зависимости от типа вопроса
...
function getCorrectAnswer() {
    switch (gameState.currentMode) {
        ...
        case 'dictation':
            return gameState.currentWord.en.toLowerCase();
        case 'mixed':
            // Для смешанного режима определяем на лету
            return gameState.currentWord.en.toLowerCase();
    }
}

// Проверка равенства ответов (с учетом возможных вариантов)
function checkAnswerEquality(userAnswer, correctAnswer) {
    // Разделяем возможные варианты перевода (если их несколько через запятую)
    const correctAnswers = correctAnswer.split(',').map(a => a.trim().toLowerCase());
    
    // Проверяем, совпадает ли ответ пользователя с одним из правильных вариантов
    ...
    if (isCorrect) {
        ...
        switch (gameState.currentMode) {
            ...
            case 'dictation':
                ...
                break;
            case 'mixed':
                points = 2; // Среднее значение
                break;
        }
        
        gameState.score += points;
        
        // Добавление слова в изученные
        ...
        // Показываем правильный ответ 2 секунды, затем следующий вопрос
        setTimeout(() => {
            gameState.selectedAnswer = null;
            showNextQuestion();
        }, 2000);
    ...
    if (isCorrect) {
        ...
        feedbackContainer.innerHTML = `
            <div class="feedback-content">
                ...
                <p>Вы заработали очки!</p>
                <p><strong>${gameState.currentWord.en}</strong> - <strong>${gameState.currentWord.ru}</strong></p>
                <p class="question-example">Пример: ${gameState.currentWord.example}</p>
            </div>
        `;
    } else {
        ...
        feedbackContainer.innerHTML = `
            <div class="feedback-content">
                ...
                <p>Ваш ответ: <strong>${userAnswer || 'не выбран'}</strong></p>
...
function showHint() {
    ...
    switch (gameState.currentMode) {
        case 'translation-ru-en':
            hintText = `Первая буква: <strong>${gameState.currentWord.en[0].toUpperCase()}</strong>`;
            break;
        case 'translation-en-ru':
            hintText = `Первая буква перевода: <strong>${gameState.currentWord.ru[0].toUpperCase()}</strong>`;
            break;
        case 'fill-blank':
            hintText = `Длина слова: <strong>${gameState.currentWord.en.length}</strong> букв`;
            break;
        case 'dictation':
            hintText = `Первая буква: <strong>${gameState.currentWord.en[0].toUpperCase()}</strong>, длина: <strong>${gameState.currentWord.en.length}</strong> букв`;
            break;
        default:
            hintText = `Попробуйте еще раз!`;
    }
    ...
    // Штраф за подсказку (минус 1 очко)
...
function startTimer() {
    ...
    gameState.timerInterval = setInterval(() => {
        ...
        if (gameState.timeLeft <= 0) {
            clearInterval(gameState.timerInterval);
            // Автоматическая проверка ответа (или пропуск)
...
function updateTimerDisplay() {
    // Можно добавить отображение таймера в UI
    // Например, добавить элемент с id="timer" и обновлять его
}
...
function getModeName(mode) {
    const modeNames = {
        'translation-ru-en': 'Рус → Англ',
        'translation-en-ru': 'Англ → Рус',
...
function displayDictionary() {
    ...
    // Собираем все слова с учетом фильтра
    for (const level in appState.words) {
    ...
    // Сортировка по английскому слову
    ...
    if (allWords.length === 0) {
        dictionaryList.innerHTML = `
            <div class="empty-message">
                ...
                <p>Слова не найдены. Добавьте новые слова или измените фильтры.</p>
            </div>
        `;
        return;
    }
    ...
    allWords.forEach(word => {
        ...
        html += `
            <div class="word-card ${learnedClass}">
                ...
                ${isLearned ? '<div class="word-learned"><i class="fas fa-check-circle"></i> Изучено</div>' : ''}
            </div>
        `;
    });
...
function addNewWord() {
    ...
    // Проверка, существует ли уже такое слово на этом уровне
    if (appState.words[level]) {
        const existingWord = appState.words[level].find(w => w.en.toLowerCase() === enWord.toLowerCase());
        if (existingWord) {
            showNotification('Это слово уже есть в словаре на этом уровне!', 'warning');
            return;
        }
    } else {
    ...
    // Уведомление
    showNotification(`Слово "${enWord}" добавлено в уровень "${level}"!`, 'success');
...
function updateWordCounts() {
    // Эта функция уже вызывается в других местах, оставляем для совместимости
    updateUIStats();
}
...
function applySettings() {
    ...
    // Можно добавить дополнительные стили для тем
...
function resetProgress() {
    showConfirmModal(
        'Сбросить прогресс?',
        'Вы уверены, что хотите сбросить всю статистику и изученные слова? Это действие нельзя отменить.',
        function() {
            appState.stats = {
                gamesPlayed: 0,
                bestScore: 0,
...
function playSound(type) {
    ...
    // В реальном приложении здесь можно добавить воспроизведение звуковых файлов
    // Для простоты используем Web Audio API для генерации простых звуков
...
function initEventListeners() {
    // Обработка нажатия Enter в поле ввода ответа
...
function updateWordCounts() {
    // Эта функция уже реализована как updateUIStats()
    updateUIStats();
}
